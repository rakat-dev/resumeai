...
// ── PDF Generator (client-side, formatted) ────────────────────────────────
function downloadPDF(resumeText: string, filename: string) {
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .section-header {
    font-size: 15pt; font-weight: bold; color: ${DARK_BLUE};
  ...
  .edu-detail { font-size: 10pt; color: ${BLACK}; }
  .project-title { font-size: 13pt; font-weight: bold; color: ${BLACK}; margin-top: 6pt; }
</style>
</head>
<body>`;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!nameWritten) {
      ...
      html += `<div class="name">${namePart}</div>`;
      nameWritten = true;
      continue;
    }
    ...
    // Title — second line
    if (!titleWritten) {
      // Could be "Senior Full Stack Engineer" or "Name — Title"
      const titleMatch = line.includes("—") ? line.split("—")[1]?.trim() : line;
      if (titleMatch && !line.includes("@") && !line.includes("|")) {
        html += `<div class="title">${titleMatch}</div>`;
        titleWritten = true;
        continue;
      }
    }
    ...
    if (!contactWritten && (line.includes("|") || line.includes("@") || line.match(/\d{3}[-.\s]\d{3}/))) {
      ...
      const display = parts.map(p => {
        if (p.match(/\d{3}/)) return p; // phone
        if (p.toLowerCase().includes("gmail") || p.toLowerCase().includes("mail")) return "Email ↗";
        return p + " ↗";
      }).join("  |  ");
      html += `<div class="contact">${display}</div>`;
      contactWritten = true;
      continue;
    }
    ...
    if (SECTION_KEYWORDS.some(k => line.toUpperCase() === k || line.toUpperCase().startsWith(k))) {
      inSkills = line.toUpperCase().includes("SKILL");
      inExperience = line.toUpperCase().includes("EXPERIENCE");
    ...
    if (line.startsWith("Tech Stack:")) {
      const rest = line.replace("Tech Stack:", "").trim();
      html += `<div class="tech"><span class="skill-bold">Tech Stack: </span>${rest}</div>`;
      continue;
    }
    ...
    if (inSkills && SKILL_CATS.some(c => line.startsWith(c + ":"))) {
      ...
      const cat = line.slice(0, colon);
      const skills = line.slice(colon + 1).trim();
      html += `<div class="skill-row"><span class="skill-bold">${cat}: </span>${skills}</div>`;
      continue;
    }

    // Company names
    if (COMPANIES.some(c => line.includes(c))) {
      const parts = line.split("|");
      const compPart = parts[0].trim();
      const datePart = parts[1]?.trim() || line.split(/\s{3,}/)[1] || "";
      html += `<div class="company">${compPart}${datePart ? `<span style="float:right;font-size:10pt;font-weight:normal">${datePart}</span>` : ""}</div>`;
      continue;
    }

    // Role lines (italic pattern)
    if (line.toLowerCase().startsWith("role:") || line.toLowerCase().startsWith("sr.") || line.toLowerCase().startsWith("software") || line.toLowerCase().startsWith("senior")) {
      if (!line.includes("•") && line.length < 120) {
        html += `<div class="role">${line}</div>`;
        continue;
      }
    }

    // Project titles (bold)
    if (line.startsWith("Project:") || (inExperience === false && line.length < 80 && !line.includes("•"))) {
      if (line.startsWith("Project:")) {
        html += `<div class="tech"><span class="skill-bold">Project: </span>${line.replace("Project:", "").trim()}</div>`;
        continue;
      }
    }

    // Education entries
    if (line.startsWith("Master") || line.startsWith("Bachelor") || line.startsWith("Doctor")) {
      html += `<div class="edu-degree">${line}</div>`;
      continue;
    }
    if (line.includes("University") || line.includes("Institute") || line.includes("College")) {
      html += `<div class="edu-school">${line}</div>`;
      continue;
    }
    if (line.startsWith("Focus:") || line.match(/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|July|April|January)\s+\d{4}/)) {
      html += `<div class="edu-detail">${line}</div>`;
      continue;
    }
  ...
  printWindow.onload = () => {
    printWindow.focus();
    printWindow.print();
    // printWindow.close(); // let user close after saving
  };
}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {ats.matched.length > 0 && (
        <div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            {ats.matched.map((k,i) => (
              <span key={i} style={{ fontSize:11, padding:"3px 10px", borderRadius:100, background:"rgba(0,229,176,0.1)", color:"var(--accent2)", border:"1px solid rgba(0,229,176,0.3)" }}>{k}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {ats.missing.length > 0 && (
        <div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
            {ats.missing.map((k,i) => (
              <span key={i} style={{ fontSize:11, padding:"3px 10px", borderRadius:100, background:"rgba(255,107,107,0.1)", color:"var(--accent3)", border:"1px solid rgba(255,107,107,0.3)" }}>{k}</span>
            ))}
          </div>
        </div>
      )}
...
export default function TailorPage() {
  ...
  const [step, setStep] = useState<0|1|2>(0); // 0=empty, 1=tailored, 2=improved
  const [copied, setCopied] = useState(false);

  // ── Tailor ────────────────────────────────────────────────────────────────
  const handleTailor = async () => {
    if (!jd.trim()) return;
    setTailoring(true); setTailorErr(""); setStep1Resume(""); setStep1Ats(null); setStep2Resume(""); setStep2Ats(null); setStep(0);
    try {
  ...
  const setCurrentResume = step === 2
    ? (v: string) => setStep2Resume(v)
    : (v: string) => setStep1Resume(v);
  ...
  const handleDownload = () => {
    const fname = step === 2 ? "Rahul_Resume_Final.pdf" : "Rahul_Resume_Tailored.pdf";
    downloadPDF(currentResume, fname);
  };
    ...
    <AppLayout>
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ color:"var(--muted)", fontSize:14, marginTop:4 }}>
          AI rewrites your resume using the JD&apos;s exact language · ATS scored · Apply suggestions for final version
        </p>
      </div>
          ...
          {step > 0 && activeTab === "resume" && (
            <div style={{ display:"flex", gap:8, marginBottom:14, flexWrap:"wrap" }}>
              ...
              <button onClick={handleDownload}
                style={{ ...S.btn("var(--surface2)","var(--text)",true), border:"1px solid var(--border)" }}>
                ⬇️ Download PDF
              </button>
              {step===2 && (
                <button onClick={handleRollback}
                  style={{ ...S.btn("rgba(255,107,107,0.1)","var(--accent3)",true), border:"1px solid rgba(255,107,107,0.3)" }}>
                  ↩ Rollback to v1
                </button>
              )}
              {step===2 && (
